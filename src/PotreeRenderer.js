...
class WebGLTexture {
  ...
  update() {
    ...
    if (texture instanceof THREE.DataTexture) {
      ...
      gl.texImage2D(this.target, level, internalFormat, internalFormat, srcType,
                    data);
...
export class Renderer {
  ...
  createBuffer(geometry) {
    ...
    for (let attributeName in geometry.attributes) {
      ...
      if (attributeLocations[attributeName] === undefined) {
        ...
        gl.vertexAttribPointer(attributeLocation, bufferAttribute.itemSize,
                               type, normalized, 0, 0);
  ...
  traverse(scene) {
    ...
    let result = {octrees : octrees};

    return result;
  }

  renderNodes(octree, nodes, visibilityTextureData, camera, target, shader,
              params) {
    ...
    for (let node of nodes) {
      ...
      // TODO consider passing matrices in an array to avoid uniformMatrix4fv
      // overhead
      ...
      { // Clip Polygons
        if (material.clipPolygons && material.clipPolygons.length > 0) {
          ...
          let flattenedMatrices =
              [].concat(...worldViewProjMatrices.map(m => m.elements));

          let flattenedVertices =
              new Array(8 * 3 * material.clipPolygons.length);
      ...
      if (shadowMaps.length > 0) {
        ...
        shader.setUniform3f("uShadowColor",
        ...
        for (let i = 0; i < shadowMaps.length; i++) {
          ...
          let glTexture =
              this.threeRenderer.properties.get(shadowMap.target.texture)
                  .__webglTexture;
...

        {
          ...
          let worldViewMatrices =
              shadowMaps.map(sm => sm.camera.matrixWorldInverse)
                  .map(view =>
                           new THREE.Matrix4().multiplyMatrices(view, world))

          let flattenedMatrices =
              [].concat(...worldViewMatrices.map(c => c.elements));
      ...
      if (geometry.attributes["gps-time"]) {
        ...
        // shader.setUniform2f("uFilterGPSTimeClipRange", [-Infinity,
        // Infinity]);
        ...
        // shader.setUniform2f("uFilterGPSTimeClipRange", [gpsCliPRangeMin,
        // gpsCliPRangeMax]);
        ...
        // // normalizedBufferRange: norm buffer within norm global range e.g.
        // [0.2, 0.8] const globalWidth = globalRange[1] - globalRange[0]; const
        // normalizedBufferRange = [ 	(bufferRange[0] - globalRange[0]) /
        // globalWidth, 	(bufferRange[1] - globalRange[0]) / globalWidth,
        // ];

        // shader.setUniform2f("uNormalizedGpsBufferRange",
        // normalizedBufferRange);

        // let uFilterGPSTimeClipRange =
        // material.uniforms.uFilterGPSTimeClipRange.value; let gpsCliPRangeMin
        // = uFilterGPSTimeClipRange[0] let gpsCliPRangeMax =
        // uFilterGPSTimeClipRange[1]
        // shader.setUniform2f("uFilterGPSTimeClipRange", [gpsCliPRangeMin,
        // gpsCliPRangeMax]);
...

      {
        ...
        shader.setUniform2f("uFilterReturnNumberRange",
                            uFilterReturnNumberRange);
        shader.setUniform2f("uFilterNumberOfReturnsRange",
                            uFilterNumberOfReturnsRange);
        shader.setUniform2f("uFilterPointSourceIDClipRange",
      ...
      let isExtraAttribute =
          ...
          Object.keys(geometry.attributes)
              .includes(material.activeAttributeName);
      ...
      if (isExtraAttribute) {
        ...
        for (const attributeName in geometry.attributes) {
          ...
          if (attributeLocations[attributeName] !== undefined) {
            ...
            let type =
                this.glTypeMapping.get(bufferAttribute.array.constructor);
            let normalized = bufferAttribute.normalized;
    ...
    if (exports.measureTimings) {
      ...
      performance.measure("render.renderNodes", "renderNodes-start",
                          "renderNodes-end");
    }
  }
  ...
  renderOctree(octree, nodes, camera, target, params = {}) {
    ...
    if (material.pointSizeType >= 0) {
      if (material.pointSizeType === PointSizeType.ADAPTIVE ||
        ...
        visibilityTextureData =
            octree.computeVisibilityTextureData(vnNodes, camera);
    ...
    { // UPDATE SHADER AND TEXTURES
      ...
      // if(material.needsUpdate){
      {
        ...
        let numClipBoxes = (material.clipBoxes && material.clipBoxes.length)
                               ? material.clipBoxes.length
                               : 0;
        let numClipSpheres = (params.clipSpheres && params.clipSpheres.length)
    ...
    { // UPDATE UNIFORMS
      ...
      if (material.clipBoxes && material.clipBoxes.length > 0) {
        // let flattenedMatrices = [].concat(...material.clipBoxes.map(c =>
        // c.inverse.elements));
        ...
        const lClipBoxes = shader.uniformLocations["clipBoxes[0]"];
        gl.uniformMatrix4fv(lClipBoxes, false,
      ...
      if (params.clipSpheres && params.clipSpheres.length > 0) {
        ...
        for (let clipSphere of clipSpheres) {
          // let mScale = new
          // THREE.Matrix4().makeScale(...clipSphere.scale.toArray()); let
          // mTranslate = new
          // THREE.Matrix4().makeTranslation(...clipSphere.position.toArray());

          // let clipToWorld = new THREE.Matrix4().multiplyMatrices(mTranslate,
          // mScale);
        ...
        // const lClipSpheres = shader.uniformLocations["uClipSpheres[0]"];
        // gl.uniformMatrix4fv(lClipSpheres, false,
        // material.uniforms.clipSpheres.value);
      }
      ...
      shader.setUniform3f("uIntensity_gbc", [
        material.intensityGamma, material.intensityBrightness,
        material.intensityContrast
      ]);

      shader.setUniform3f(
          "uRGB_gbc",
          [ material.rgbGamma, material.rgbBrightness, material.rgbContrast ]);
      ...
      shader.setUniform("backfaceCulling",
      ...
      if (repeat === ElevationGradientRepeat.REPEAT) {
        ...
        gl.texParameteri(gradientTexture.target, gl.TEXTURE_WRAP_S,
                         gl.MIRRORED_REPEAT);
        gl.texParameteri(gradientTexture.target, gl.TEXTURE_WRAP_T,
                         gl.MIRRORED_REPEAT);
      } else {
        gl.texParameteri(gradientTexture.target, gl.TEXTURE_WRAP_S,
                         gl.CLAMP_TO_EDGE);
      ...
      let classificationTexture =
          this.textures.get(material.classificationTexture);
      ...
      if (material.snapEnabled === true) {

        {
          ...
          let lSnapshotBindingPoints =
              new Array(5).fill(bindingStart).map((a, i) => (a + i));
          let lSnapshotDepthBindingPoints =
              new Array(5)
    ...
    this.renderNodes(octree, nodes, visibilityTextureData, camera, target,
                     shader, params);
  ...
  render(scene, camera, target = null, params = {}) {
    ...
    // TODO: uncomment. only run if Viewer.lkg == false.
